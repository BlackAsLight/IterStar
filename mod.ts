...
export class Iter<T> {
	...
	map<U>(func: (x: T) => U): Iter<U> {
		return new Iter(
			(function* (iter) {
				for (const x of iter) yield func(x)
			})(this),
		)
	}
	...
	flat(): Iter<Flat<T>> {
		return new Iter(
			(function* (iter) {
				for (const x of iter)
					if ((x as Force)[Symbol.iterator]) for (const y of new Iter(x as Force).flat()) yield y
					else yield x
			})(this),
		) as Force
	}
	...
	filter<U, V extends unknown = boolean>(func: (x: T) => V): Iter<Extract<T, U>> {
		return new Iter(
			(function* (iter) {
				for (const x of iter) if (func(x)) yield x as Extract<T, U>
			})(this),
		)
	}
...
export class AsyncIter<T> {
	...
	constructor(array: { [Symbol.asyncIterator](): AsyncIterator<T> }) {
		this.#gen = (async function* () {
			for await (const x of array) yield x
		})()
	}
	...
	async forEach<U>(func: (x: T) => U): Promise<void> {
		for await (const x of this.#gen) func(x)
	}

	map<U>(func: (x: T) => U): AsyncIter<U> {
		return new AsyncIter(
			(async function* (asyncIter) {
				for await (const x of asyncIter) yield func(x)
			})(this),
		)
	}
		...
		return new AsyncIter(
			(async function* (asyncIter) {
				for await (const x of asyncIter)
					if ((x as Force)[Symbol.asyncIterator]) for await (const y of new AsyncIter(x as Force).flat()) yield y
					else yield x
			})(this),
		) as Force
	}
	...
	filter<U, V extends unknown = boolean>(func: (x: T) => V): AsyncIter<Extract<T, U>> {
		return new AsyncIter(
			(async function* (asyncIter) {
				for await (const x of asyncIter) if (func(x)) yield x as Extract<T, U>
			})(this),
		)
	}
...
export class Queue<T> {
	...
	constructor(arrayLike?: { [Symbol.iterator](): Iterator<T> }) {
		if (arrayLike) for (const value of arrayLike) this.#list[this.#tail++] = value
	}
	...
	push(...array: T[]): number {
		for (const value of array) this.#list[this.#tail++] = value
		return this.length
	}
